                ...
                for data_s in root.iter("ECUC-CONTAINER-VALUE"):
                    ...
                    if def_Val is not None and def_Val.text.endswith("Dcm/DcmConfigSet/DcmDsp/DcmDspData"):
                        ...
                        if did_dataInfo.text == did_dataRef:
                            ...
                            did_ReadFunction = find_recursive_Value(data_s, "DEFINITION-REF", "DcmDspDataReadFnc")
                            did_WriteFunction = find_recursive_Value(data_s, "DEFINITION-REF", "DcmDspDataWriteFnc")
                            size = ""
                            Read_fct = ""
                            Write_fct = ""
                            ...
                            if did_ReadFunction is not None:
                                Read_fct = did_ReadFunction
                            if did_WriteFunction is not None:
                                Write_fct = did_WriteFunction
                            did_data.append({"DID": did_id, "Size" : size, "Read": Read_fct, "Write": Write_fct})
                            break
                ...
                for data_s in root.iter("ECUC-CONTAINER-VALUE"):
                    ...
                    if def_Val is not None and def_Val.text.endswith("Dcm/DcmConfigSet/DcmDsp/DcmDspRoutineInfo"):
                        ...
                        if rc_dataInfo.text == rc_info:
                            ...
                            rc_data.append({"RC ID": rc_id, "Start RC": start_fct, "Start DataIn": rc_start_dataIn_size, "Start DataOut": rc_start_dataOut_size, \
                                                            "Stop RC": stop_fct, "Stop DataIn": rc_stop_dataIn_size, "Stop DataOut": rc_stop_dataOut_size, \
                                                            "Result RC": result_fct, "Result DataIn": rc_result_dataIn_size, "Result DataOut": rc_result_dataOut_size})
                            break
    else:
